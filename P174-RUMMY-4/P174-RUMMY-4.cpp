...
int nuevaJugada(tSoporte soporte, tJugada jugada) {
	...
	*/
	...
	else if (n_fichas == 1) { // Pone solo una ficha
...
bool ponerFicha(tJugada jugada, tFicha ficha) {
	/*
	Intenta colocar la ficha en la jugada en ambos extremos
	Devuelve true si la coloca y false si no
	jugada: array de fichas(color y contador)
	ficha: color y valor
	*/
	bool colocada = false; // Variable que indica si se ha colocado o no la ficha
	int i = 0; // Variable que se usa para recorrer las fichas de la jugada
	// Bucle para saber la posicion de la ultima ficha en la jugada
	while (jugada[i].valor != -1) {
		if (jugada[i].valor != -1) {
			i++;
		}
	}
	// Si la ficha va en la primera posicion de la jugada
	if (ficha.valor < jugada[0].valor) {
		// Se desplazan todas las fichas una posicion a la derecha
		for (int j = i; j > 0; j--) {
			jugada[j] = jugada[j - 1];
		}
		// Y se coloca la ficha en la primera posicion de la jugada
		jugada[0] = ficha;
		colocada = true;
	}
	// Si la ficha va en la ultima posicion de la jugada
	else if (jugada[i].valor < ficha.valor) {
		// Se coloca en la primera posicion de la jugada
		jugada[i] = ficha;
		colocada = true;
	}
	return colocada;
...
bool jugar(tTablero& tablero, tSoporte& soporte) {
	/*
	Permite al usuario colocar fichas de su soporte en el tablero, ya sean en nuevas jugadas
	o en jugadas existentes
	tablero: array de jugadas(array de fichas) y contador
	soporte: array de fichas(color y contador) y contador
	*/
	tJugada jugada; // Variable en la que guardar la nueva jugada
	iniJugada(jugada); // Se inicializa la jugada
	bool colocada = false; // Variable que indica si se ha colocado o no la ficha
	bool encontrado = false; // Variable que indica si se ha encontrado la jugada en el tablero o no
	int i = 0; // Variable con la que recorrer las jugadas del tablero
	int n_fichas = nuevaJugada(soporte, jugada);
	if (n_fichas != -1) { // Si la jugada tiene alguna ficha
		if (n_fichas == 1) { // Si la jugada tiene solo una ficha
			cout << "Jugadas del tablero donde poner la ficha: ";
			while ((i < tablero.contador) && !encontrado) {
				if (tablero.jugadas[i][0].color == jugada[0].color) {
					encontrado = true;
				}
				if (!encontrado) {
					i++;
...
bool resuelveCaso() {
	...
	do {
		...
		case 4: // Se ordena el soporte por color segun los ejemplos del juez y permite crear una nueva jugada
			...
			cout << endl;
			juega = jugar(tablero, soportes[turno - 1]);
			if (!juega) {
				n_fichas = -1;
			}
			else if (juega) {
			...
			cout << endl << endl;
			mostrar(soportes[turno - 1]);
			if (soportes[turno - 1].contador == 0) {
				ganar = true;
			}
			cout << endl;
			break;
